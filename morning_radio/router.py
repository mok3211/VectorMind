...
from morning_radio.agent import agent
...
router = APIRouter(prefix="/morning-radio", tags=["morning-radio"])
...
async def generate(req: GenerateReq, _: object = Depends(require_permission("agents.run")), session=Depends(get_session)):
    try:
        ...
        result = await agent.run(topic=req.topic, audience=req.audience, llm_model=llm_model, llm_extra=llm_extra)
        session.add(
            AgentRun(
                agent="morning_radio",
...
class PublishReq(BaseModel):
    publisher: str  # markdown / xhs / douyin
    title: str
    text: str
